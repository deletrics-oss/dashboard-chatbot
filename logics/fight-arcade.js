...
const attachLogic = (client, io, clientSession, addEntry) => {
    ...
    client.on('message', async (msg) => {
        ...
        if (messageBody.match(/^(oi|olá|ola|menu|começar)/i)) {
            ...
            await sendMessage(user, `Olá, ${contactName.split(" ")[0]}! 👋 Sou o assistente virtual da Fight Arcade.`);
        ...
        switch (userStates[user]?.stage) {
            case 'aguardando_menu_principal':
                switch(messageBody) {
                    case '1':
                        await sendMessage(user, "Legal! Para começar, diga o que procura:\n\n*1️⃣ - Fliperama Completo*\n*2️⃣ - Apenas Controle USB*\n\n*0️⃣ - Voltar ao menu principal*");
                        userStates[user].stage = 'aguardando_tipo_produto';
                        break;
                    // Adicione aqui os outros casos para o menu principal (2, 3, 4, 5)
                    default:
                        await sendMessage(user, "Opção inválida. Por favor, digite um número de 1 a 5.");
                        break;
                }
                break;
            ...
            case 'aguardando_tipo_produto':
                ...
                await sendMessage(user, `Entendido! E precisa para quantos jogadores?\n\n*1️⃣ - 1 Jogador*\n*2️⃣ - 2 Jogadores*\n\n*0️⃣ - Voltar*`);
                userStates[user].stage = 'aguardando_jogadores';
                break;

            case 'aguardando_jogadores':
                if (messageBody === '0') {
                    await sendMessage(user, "Ok, a voltar. O que procura?\n\n*1️⃣ - Fliperama Completo*\n*2️⃣ - Apenas Controle USB*\n\n*0️⃣ - Voltar ao menu principal*");
                    userStates[user].stage = 'aguardando_tipo_produto';
                    break;
                }
                userStates[user].jogadores = messageBody.charAt(0);
                await sendMessage(user, `Ótimo! E qual material prefere?\n\n*1️⃣ - MDF*\n*2️⃣ - Metal*\n\n*0️⃣ - Voltar*`);
                userStates[user].stage = 'aguardando_material';
                break;
            
            case 'aguardando_material':
                if (messageBody === '0') {
                    await sendMessage(user, `Ok, a voltar. Para quantos jogadores?\n\n*1️⃣ - 1 Jogador*\n*2️⃣ - 2 Jogadores*\n\n*0️⃣ - Voltar*`);
                    userStates[user].stage = 'aguardando_jogadores';
                    break;
                }
